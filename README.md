...
The app checks cache paths with Btrfs subvolume listings under `source.cache_root` and, for deletion, under each timestamp cache parent. Bulk-list path resolution accepts filesystem-relative prefixes that are not present in the configured mount path, such as an on-disk `@/` prefix. This lets the same cache subvolumes be found after the Btrfs tree is remounted below a different directory and prevents normal Timeshift paths with the same date/name from being mistaken for app-created send-cache snapshots.

Before creating `<cache_root>/<snapshot>/<subvolume>`, the app probes that exact path inside the same source command that would create and verify the cache snapshot. If the exact path already exists, is read-only, and its Parent UUID matches the original Timeshift subvolume when both UUIDs are available, it is reused immediately even if the earlier bulk cache index missed it. Only an absent exact target is passed to `btrfs subvolume snapshot -r`. This is important because Btrfs treats an existing destination as a directory and appends the source basename; passing an existing `<date>/@` as the destination would otherwise attempt `<date>/@/@` and fail because the existing cache snapshot is read-only.

A missing exact-path probe is expected when the cache snapshot has not been created yet, for example `send-cache/<snapshot>/@home`. Existing ordinary paths, writable cache subvolumes, and cache snapshots whose Parent UUID belongs to a different Timeshift original are refused instead of overwritten or nested. The exact probe, optional create, and confirming metadata read use one source command/SSH session; required failures and real send/receive failures still print and log full stderr.
