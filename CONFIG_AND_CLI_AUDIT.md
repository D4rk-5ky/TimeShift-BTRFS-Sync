...
- Destination metadata is written once after all configured subvolumes for the date are complete; paired `@`/`@home`, root-only, and home-only configurations use the same behavior. Recovery and prune remove only the specific copied file after destination subvolume cleanup.
