...
class ActionKind(str, Enum):
    ...
    DELETE_SOURCE_TIMESHIFT = "delete-source-timeshift"
...
def plan_prune_snapshot(
    snapshot_name: str,
    *,
    delete_cache: bool,
    delete_destination: bool,
    delete_source_timeshift: bool = False,
) -> WorkflowPlan:
    """Plan one retention deletion in source/destination/cache/state order.

    Source Timeshift deletion is a separate action because it is allowed only
    for app-created on-demand snapshots and must be performed through Timeshift
    itself, never by raw Btrfs tree deletion. Before this plan runs, retention
    must already have proven a complete live backup for any current source
    snapshot. Deleting the source first keeps that backup available when a
    Timeshift deletion fails, so the next prune can safely re-prove and retry.
    """

    plan = WorkflowPlan("prune")
    if delete_source_timeshift:
        plan.add(ActionKind.DELETE_SOURCE_TIMESHIFT, snapshot=snapshot_name)
