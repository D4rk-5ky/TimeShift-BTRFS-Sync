...
Available commands:
  ...
  prune          Apply backup retention and guarded app-created source snapshot retention.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = new_subparser(
        ...
        (
            ...
            "The remote-roundtrip profile is preconfigured for SSH Timeshift source -> local backup "
            "-> push restore to the same SSH Timeshift host."
        ),
        cmd_init_config,
    )
    ...
    p.add_argument("--prune", action="store_true", help="run retention pruning after sync; real delete also needs --run --yes-delete")
    ...
    p = new_subparser(
        ...
        (
            "Apply retention rules to destination backups and, for app-created on-demand snapshots, the matching source Timeshift snapshots.\n"
            "Source deletion is limited to current tag O snapshots that still match manual_snapshot.marker and is performed through Timeshift itself.\n"
