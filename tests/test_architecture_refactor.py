...
from timeshift_btrfs_sync import destroy, inventory, remote_index, retention, sync, tree_ops
...
from timeshift_btrfs_sync.snapshot_records import build_backup_inventory
...
class ArchitectureRefactorTests(unittest.TestCase):
    def combined_inventory(self):
        source_index = inventory.BtrfsIndex("/snapshots", "source")
        cache_index = inventory.BtrfsIndex("/cache", "source")
        destination_index = inventory.BtrfsIndex("/target", "local")
        source_snapshots = {DATE_1: snapshot(DATE_1), DATE_2: snapshot(DATE_2)}
        for snap in source_snapshots.values():
            for meta in snap.subvolumes.values():
                source_index.add(meta)
        cache_index.add(SubvolumeMeta("@", f"/cache/{DATE_1}/@", uuid="cache-1", readonly=True))
        destination_index.add(
            SubvolumeMeta("@", f"/target/snapshots/{DATE_1}/@", received_uuid="cache-1", readonly=True)
        )
        source_inventory = inventory.SourceInventory(
            f"{DATE_1}\n{DATE_2}",
            source_index,
            cache_index,
            {DATE_1: '{"date":"one"}', DATE_2: '{"date":"two"}'},
        )
        state = {
            "snapshots": {
                DATE_1: {
        ...
        return build_backup_inventory(
            ...
            subvolume_names=["@", "@home"],
        )

    def test_remote_index_is_only_a_compatibility_view_of_inventory(self):
        self.assertIs(remote_index.BtrfsIndex, inventory.BtrfsIndex)
        self.assertIs(remote_index.SourceInventory, inventory.SourceInventory)
        self.assertIs(remote_index.build_source_inventory, inventory.build_source_inventory)

    def test_combined_snapshot_record_joins_all_four_views(self):
        combined = self.combined_inventory()
        record = combined.get(DATE_1)
        self.assertIsNotNone(record)
        assert record is not None
        self.assertEqual(record.source_meta("@").uuid, f"source-{DATE_1}")
        self.assertEqual(record.cache["@"].uuid, "cache-1")
        self.assertEqual(record.destination["@"].received_uuid, "cache-1")
        self.assertEqual(record.state["subvolumes"]["@"]["status"], "ok")
        self.assertEqual(record.info_json, '{"date":"one"}')

    def test_sync_planner_is_pure_and_orders_oldest_to_newest(self):
        combined = self.combined_inventory()
        before = repr(combined.state)
        plan = plan_sync_queue(combined, [DATE_2, DATE_1], ["@", "@home"])
        self.assertEqual(repr(combined.state), before)
    ...
    def test_planner_and_executor_compose_in_the_same_order(self):
        plan = plan_sync_queue(self.combined_inventory(), [DATE_2, DATE_1], ["@", "@home"])
    ...
    def test_operation_modules_have_single_authoritative_definitions(self):
        ...
        self.assertEqual(definitions["build_backup_inventory"], ["snapshot_records.py"])
        self.assertEqual(definitions["WorkflowExecutor"], ["executor.py"])

    def test_refactor_reduces_runtime_source_size_and_function_count(self):
        package = Path("timeshift_btrfs_sync")
        ...
        # Uploaded 0.1.46 baseline: 11,645 package lines and 408 functions.
        self.assertLess(line_count, 11_645)
        self.assertLess(function_count, 408)
