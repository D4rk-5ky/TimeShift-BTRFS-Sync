...
class RemoteBackupRestoreTests(unittest.TestCase):
    ...
    def test_remote_backup_streams_ssh_send_into_local_receive(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch.object(restore, "_read_privileged_file", return_value=INFO),
    ...
    def test_pull_restore_live_common_proof_probes_local_timeshift_cache_path(self):
        config = self.make_config(Path("/tmp"))
        ...
        proven, reason = restore._live_payload_proof(
            config,
            SubvolumeMeta("@", f"{config.source.snapshot_root}/{SNAPSHOT}/@", uuid="original-root"),
            SubvolumeMeta(
                "@",
                f"{config.destination.target_root}/snapshots/{SNAPSHOT}/@",
                uuid="backup-root",
                received_uuid="send-root",
                readonly=True,
            ),
            ops,
            inventory,
        ...
        self.assertEqual(cache_path, ops.paths[0])
    ...
    def test_remote_backup_scan_never_uses_local_timeshift_cache_root(self):
        ...
        output = "\n".join([
            f"TSBTRFS_REPOSITORY_DATE\t{SNAPSHOT}\tdirectory",
            f"TSBTRFS_REPOSITORY_ENTRY\t{SNAPSHOT}\t@\tdirectory",
            f"TSBTRFS_REPOSITORY_ENTRY\t{SNAPSHOT}\t@home\tdirectory",
            f"TSBTRFS_REPOSITORY_ENTRY\t{SNAPSHOT}\tinfo.json\tfile",
            f"TSBTRFS_REPOSITORY_INFO\t{SNAPSHOT}\t{encoded}",
        ]) + "\n"
        config = self.make_config(Path("/tmp"))
    ...
    def test_timeshift_path_probe_uses_source_privilege_prefix(self):
        config = self.make_config(Path("/tmp"))
        endpoint = FakeEndpoint("")
        endpoint.argv_result = Completed(1, "", "")

        exists, error = restore._source_path_exists(
            endpoint,
            config,
            "/local/timeshift-btrfs/snapshots/example",
        )
        ...
        self.assertEqual(
            endpoint.argv_calls,
            [["sudo", "-n", "test", "-e", "/local/timeshift-btrfs/snapshots/example"]],
        )

    def test_repository_scan_uses_configured_privilege_prefix(self):
        encoded = base64.b64encode(INFO.encode()).decode()
        output = "\n".join([
            f"TSBTRFS_REPOSITORY_DATE\t{SNAPSHOT}\tdirectory",
            f"TSBTRFS_REPOSITORY_ENTRY\t{SNAPSHOT}\t@\tdirectory",
        ...
        endpoint = FakeEndpoint(output)
        records = restore._scan_snapshot_directories(
            endpoint,
            "/local/timeshift-btrfs/snapshots",
            location_label="local Timeshift repository",
            privilege_prefix=["sudo", "-n"],
        )
        self.assertIn(SNAPSHOT, records)
        self.assertIn("sudo -n", endpoint.scripts[0])
        self.assertIn("run_privileged ls -1A", endpoint.scripts[0])
        self.assertNotIn("sudo -n sh -c", endpoint.scripts[0])
        self.assertIn("/local/timeshift-btrfs/snapshots", endpoint.scripts[0])

    def test_local_physical_scanner_executes_and_preserves_info_json(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "snapshots"
            date_root = root / SNAPSHOT
            date_root.mkdir(parents=True)
            (date_root / "@").mkdir()
            (date_root / "@home").mkdir()
            (date_root / "info.json").write_text(INFO, encoding="utf-8")

            records = restore._scan_snapshot_directories(
                CommandEndpoint.local("test repository"),
                str(root),
                location_label="test repository",
                privilege_prefix=[],
            )

        self.assertEqual(records[SNAPSHOT].kind, "directory")
        self.assertEqual(
            records[SNAPSHOT].entries,
            {"@": "directory", "@home": "directory", "info.json": "file"},
        )
        self.assertEqual(records[SNAPSHOT].info_content, INFO)
        self.assertIsNone(records[SNAPSHOT].info_error)
    ...
    def test_privileged_file_reader_executes_and_preserves_content(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "state.json"
            content = '{"version":3,"snapshots":{}}\n'
            path.write_text(content, encoding="utf-8")

            actual = restore._read_privileged_file(
                CommandEndpoint.local("test repository"),
                str(path),
                location_label="test state",
                privilege_prefix=[],
            )
    ...
    def test_remote_backup_state_is_read_and_validated_on_backup_endpoint(self):
        output = 'TSBTRFS_FILE_CONTENT\t' + base64.b64encode(b'{"version":3,"snapshots":{}}').decode() + '\n'
        config = self.make_config(Path("/tmp"))
        runner = FakeRunner(ssh=True)
        endpoint = FakeEndpoint(output)
        repository = restore.BackupRepository(config, runner, endpoint, SimpleNamespace())
        ...
        self.assertIn("sudo -n", endpoint.scripts[0])
        self.assertIn("base64 -w 0", endpoint.scripts[0])
        self.assertNotIn("sudo -n sh -c", endpoint.scripts[0])
