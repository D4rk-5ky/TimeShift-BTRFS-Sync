...
TEMPLATES = {
    ...
    "remote-roundtrip": DATA / "config.remote-roundtrip.example.toml",
}
...
class ConfigProfileTests(unittest.TestCase):
    ...
    def test_all_profiles_are_packaged_resources(self):
        data = files("timeshift_btrfs_sync").joinpath("data")
        for name in ("config.example.toml", "config.restore-pull.example.toml", "config.remote-roundtrip.example.toml"):
            text = data.joinpath(name).read_text(encoding="utf-8")
            self.assertIn("[ssh]", text)
            self.assertIn("password_file", text)
            self.assertIn("identity_passphrase_file", text)
    ...
    def test_init_config_remote_roundtrip_profile_matches_packaged_template(self):
        self.assertEqual(
            self._write_profile("remote-roundtrip"),
            TEMPLATES["remote-roundtrip"].read_text(encoding="utf-8"),
        )

    def test_remote_roundtrip_profile_uses_same_ssh_host_for_sync_and_push_restore(self):
        config = load_config(TEMPLATES["remote-roundtrip"])
        self.assertEqual(config.source.mode, "ssh")
        self.assertEqual(config.restore.mode, "ssh-target")
    ...
    def test_every_current_option_is_documented_in_all_profiles(self):
        expected = {
            None: TOP_LEVEL_KEYS - {
