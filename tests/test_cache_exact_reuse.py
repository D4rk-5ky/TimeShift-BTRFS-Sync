...
from timeshift_btrfs_sync.cache_ops import CacheManager, validate_cache_snapshot
...
class CacheExactReuseTests(unittest.TestCase):
    def test_stale_bulk_index_reuses_exact_existing_cache_snapshot_without_nested_name(self) -> None:
        ...
        output = f"""TSBTRFS_CACHE_PARENT_LIST_STATUS	0
TSBTRFS_CACHE_PARENT_LIST_BEGIN
ID 20 gen 1 top level 5 parent_uuid original-uuid received_uuid - uuid cache-uuid path @
...
"""
        source = FakeSource(Completed(0, output, ""))
        ...
        self.assertIn(f"subvolume list -u -q -R -o {cache_parent}", source.calls[0])
        self.assertIn(f"subvolume list -r -o {cache_parent}", source.calls[0])
        self.assertNotIn("subvolume snapshot", source.calls[0])
    ...
    def test_root_scoped_bare_relative_cache_child_is_indexed_and_reused(self) -> None:
        cache_root = "/media/darkyere/OS-Root/timeshift-btrfs/.ts-btrfs-sync/send-cache"
        snapshot_name = "2026-06-23_07-10-24"
        child = f"{cache_root}/{snapshot_name}/@"
        output = (
            "ID 20 gen 1 top level 5 parent_uuid original-uuid "
            "received_uuid - uuid cache-uuid path "
            f"{snapshot_name}/@\n"
        )
        readonly = f"ID 20 gen 1 top level 5 path {snapshot_name}/@\n"

        metas = inventory.parse_subvolume_list(output, cache_root)

        self.assertEqual([meta.path for meta in metas], [child])
        self.assertIn(child, inventory.parse_subvolume_paths(readonly, cache_root))

    def test_existing_cache_without_parent_uuid_is_refused(self) -> None:
        original = SubvolumeMeta("@", "/snapshots/date/@", uuid="original-uuid", readonly=False)
        cache = SubvolumeMeta("@", "/cache/date/@", uuid="cache-uuid", parent_uuid=None, readonly=True)
        with self.assertRaisesRegex(RuntimeError, "no readable Parent UUID"):
            validate_cache_snapshot(cache, cache_path=cache.path, original=original)
...
"""
        ...
        self.assertEqual(cache_result.path, "/cache/date/@")
        self.assertTrue(cache_result.readonly)
        self.assertEqual(cache_result.uuid, "concurrent-cache-uuid")
