...
from timeshift_btrfs_sync.commands import CommandError, Completed
...
class RestoreSnapshotTests(unittest.TestCase):
    ...
    def run_execution(self, *, remote: bool, plan: restore.RestorePlan, inputs: list[str]):
        ...
        with (
            ...
            patch.object(restore, "_read_privileged_file", return_value=info(S1)),
    ...
    def test_build_all_plan_with_retained_cache_full_seeds_exact_common_backup(self):
        """Destination-only reset keeps cache but still needs a receiver-side full seed."""
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = {
                S1: SnapshotMeta(
                    ...
                    subvolumes={
                        ...
                        "@home": SubvolumeMeta(
                            "@home", f"/snapshots/{S1}/@home", uuid=f"original-{S1}-home"
                        ),
                    },
                )
            }
            ...
            for payload, suffix in (("@", "root"), ("@home", "home")):
                ...
                cache_index.add(
                    SubvolumeMeta(
                        payload,
                        cache_path,
                        uuid=f"send-{S1}-{suffix}",
                        parent_uuid=f"original-{S1}-{suffix}",
                        readonly=True,
                    )
                )
            ...
            repository = SimpleNamespace(load_state=lambda: {"version": 1, "snapshots": {}})
        ...
        self.assertEqual(plan.chain_names, [S1, S2, S3])
        self.assertEqual(plan.restore_names, [S2, S3])
        self.assertIn("full hidden receive", plan.seed_reason)
        self.assertIn("live cache/Btrfs lineage proves every configured payload", plan.common_reason)

    def test_same_date_info_identity_fallback_ignores_changed_tags_and_comments(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2, S3)}
            source = {
                S2: SnapshotMeta(
                    ...
                    f"/snapshots/{S2}",
                    subvolumes={
                        "@": SubvolumeMeta("@", f"/snapshots/{S2}/@", uuid=f"current-{S2}-root"),
                        "@home": SubvolumeMeta("@home", f"/snapshots/{S2}/@home", uuid=f"current-{S2}-home"),
                    },
                )
            }
            changed_info = info(S2).replace('"tags":"ondemand"', '"tags":"hourly daily"').replace('"comments":""', '"comments":"retagged later"')
            inventory = SourceInventory(
                "timeshift list",
                BtrfsIndex("/source/timeshift-btrfs/snapshots", "local"),
                BtrfsIndex("/source/timeshift-btrfs/.ts-btrfs-sync/send-cache", "local"),
                {S2: changed_info},
            )
            config = self.make_config(root, remote=False)
            repository = SimpleNamespace(load_state=lambda: {"version": 3, "snapshots": {}})
            with (
                patch.object(restore, "_discover_backups", return_value=backups),
                patch.object(restore, "_timeshift_snapshots", return_value=(source, inventory)),
            ):
        ...
        self.assertEqual(plan.common_parent, S2)
        self.assertEqual(plan.restore_names, [S3])
        self.assertIn("same physical timestamp exists", plan.common_reason)
        self.assertIn("info.json match sys-uuid", plan.common_reason)

    def test_invalid_state_is_ignored_when_same_date_info_proves_common_parent(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2, S3)}
            source = {
            ...
            repository = SimpleNamespace(
                load_state=lambda: (_ for _ in ()).throw(restore.RestoreError("invalid state schema"))
            )
            with (
                patch.object(restore, "_discover_backups", return_value=backups),
                patch.object(restore, "_timeshift_snapshots", return_value=(source, inventory)),
        ...
        self.assertEqual(plan.restore_names, [S3])
        self.assertIn("same physical timestamp exists", plan.common_reason)
        self.assertIn("state.json was ignored", plan.common_reason)
        self.assertIn("invalid state schema", plan.common_reason)

    def test_older_unreadable_dates_do_not_hide_newest_same_date_common_parent(self):
        """Old source metadata failures must not overwrite a newer valid overlap."""

        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2, S3)}
            source = {
                S1: SnapshotMeta(S1, f"/snapshots/{S1}"),
                S3: SnapshotMeta(
                    S3,
                    f"/snapshots/{S3}",
                    subvolumes={
                        "@": SubvolumeMeta("@", f"/snapshots/{S3}/@", uuid=f"current-{S3}-root"),
                        "@home": SubvolumeMeta(
                            "@home", f"/snapshots/{S3}/@home", uuid=f"current-{S3}-home"
                        ),
                    },
                ),
            }
            inventory = SourceInventory(
                ...
                BtrfsIndex("/source/timeshift-btrfs/.ts-btrfs-sync/send-cache", "local"),
                {S3: info(S3)},
                snapshot_info_errors={S1: "info.json was not found or was not readable"},
            )
            config = self.make_config(root, remote=False)
            repository = SimpleNamespace(load_state=lambda: {"version": 3, "snapshots": {}})
            with (
        ...
        self.assertEqual(plan.common_parent, S3)
        self.assertEqual(plan.restore_names, [])
        self.assertIn("same physical timestamp exists", plan.common_reason)
        self.assertNotIn(S1, plan.common_reason)
    ...
    def test_latest_common_parent_uses_state_and_live_endpoint_uuid_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2)}
            source = {
                name: SnapshotMeta(
                    ...
                    subvolumes={
                        ...
                        "@home": SubvolumeMeta(
                            "@home", f"/snapshots/{name}/@home", uuid=f"original-{name}-home"
                        ),
                    },
                )
                for name in (S1, S2)
            ...
            inventory = SourceInventory(
                "timeshift list",
                BtrfsIndex(config.source.snapshot_root, "local"),
                BtrfsIndex(config.source.cache_root, "local"),
            )
            source_ops = restore.BtrfsOps(restore.CommandEndpoint.local("source"), "sudo -n", "btrfs")
            ...
            self.assertEqual(common, S2)
            self.assertIn("same physical timestamp exists", _reason)
            self.assertIn("info.json match sys-uuid", _reason)

    def test_latest_common_parent_rejects_newer_candidate_when_uuid_and_info_proofs_both_fail(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2)}
            source = {
            ...
            state = {
                "snapshots": {
                    name: {
                        "subvolumes": {
                            "@": {"status": "ok", "original_source_uuid": f"original-{name}-root", "send_source_uuid": f"send-{name}-root"},
                            "@home": {"status": "ok", "original_source_uuid": f"original-{name}-home", "send_source_uuid": f"send-{name}-home"},
                        }
                    }
                    for name in (S1, S2)
                }
            }
            backups[S2].payloads["@home"].received_uuid = "wrong-send-uuid"
            config = self.make_config(root, remote=False)
            inventory = SourceInventory(
                "timeshift list",
                BtrfsIndex(config.source.snapshot_root, "local"),
            ...
            common, reason = restore._find_latest_common_parent(
                ...
                source,
                {
                    S1: backups[S1].os_identity,
                    S2: restore.TimeshiftOsIdentity("different-os", "btrfs", "Ubuntu"),
                },
                state,
                timeshift_ops=restore.BtrfsOps(restore.CommandEndpoint.local("source"), "sudo -n", "btrfs"),
                timeshift_inventory=inventory,
            )
        self.assertEqual(common, S1)
        self.assertIn("exact state/Btrfs UUID proof", reason)

    def test_latest_common_parent_does_not_fail_on_info_json_provenance_change(self):
        """Scheduled/retained metadata must not overrule exact Btrfs lineage."""
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_identities = {
                S1: backups[S1].os_identity,
                S2: restore.TimeshiftOsIdentity("later-filesystem-uuid", "btrfs", "Ubuntu"),
            }
            config = self.make_config(root, remote=False)
        ...
        self.assertEqual(common, S2)
        self.assertIn("exact state/Btrfs UUID proof", reason)
        self.assertIn("info.json identity is unavailable/different", reason)
        self.assertIn("exact Btrfs lineage is authoritative", reason)

    def test_uuid_proven_common_parent_overrides_repository_info_json_mismatch(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2, S3)}
            source = {
                S2: SnapshotMeta(
                    ...
                    subvolumes={
                        "@": SubvolumeMeta("@", f"/snapshots/{S2}/@", uuid=f"original-{S2}-root"),
                        "@home": SubvolumeMeta(
                            "@home", f"/snapshots/{S2}/@home", uuid=f"original-{S2}-home"
                        ),
                    },
                )
            }
                ...
                "snapshots": {
                    S2: {
                        "subvolumes": {
                            "@": {
                                "status": "ok",
                                "original_source_uuid": f"original-{S2}-root",
                                "send_source_uuid": f"send-{S2}-root",
                            },
                            "@home": {
                                "status": "ok",
                                "original_source_uuid": f"original-{S2}-home",
                                "send_source_uuid": f"send-{S2}-home",
                            },
                        }
                    }
                }
            ...
            config = self.make_config(root, remote=False)
            repository = SimpleNamespace(load_state=lambda: state)
            mismatching_info = info(S2).replace("source-root-uuid", "later-filesystem-uuid")
            with (
                patch.object(restore, "_discover_backups", return_value=backups),
                patch.object(
                    restore,
        ...
        self.assertEqual(plan.common_parent, S2)
        self.assertTrue(plan.os_identity_match)
        self.assertIn("independently proven", plan.os_identity_reason)
        self.assertIn("diagnostic only", plan.os_identity_reason)
    ...
    def test_common_parent_always_full_seeds_hidden_chain_in_local_and_ssh_target_modes(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            backups = {name: self.backup(root, name) for name in (S1, S2, S3)}
            for remote in (False, True):
                with self.subTest(remote=remote):
                    plan = restore.RestorePlan(
                        ...
                        restore_names=[S2, S3],
                        common_parent=S1,
                        common_reason="live Btrfs lineage matches",
                        no_common_parent=False,
                        seed_reason="full hidden seed required on receiver",
                    ...
                    self.assertTrue(all("-p" in pipeline[0] for pipeline in pipelines[2:]))
                    for offset, payload in enumerate(("@", "@home")):
                        self.assertTrue(pipelines[offset][0][-1].endswith(f"/snapshots/{S1}/{payload}"))
                        command = pipelines[2 + offset][0]
                        parent_index = command.index("-p") + 1
    ...
    def test_restore_inventory_includes_physical_snapshot_omitted_by_timeshift_list(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            config = self.make_config(root, remote=False)
            snapshot_index = BtrfsIndex(config.source.snapshot_root, "local")
            snapshot_index.add(
                SubvolumeMeta(
                    "@",
                    f"{config.source.snapshot_root}/{S1}/@",
                    uuid=f"original-{S1}-root",
                    readonly=False,
                )
            )
            snapshot_index.add(
                SubvolumeMeta(
                    "@home",
                    f"{config.source.snapshot_root}/{S1}/@home",
                    uuid=f"original-{S1}-home",
                    readonly=False,
                )
            )
            inventory = SourceInventory(
            ...
            records = {
                S1: restore.BackupDirectoryRecord(S1, "directory", {"@": "directory", "@home": "directory", "info.json": "file"}, info(S1))
            }
            with (
                patch.object(restore, "build_source_inventory", return_value=inventory),
                patch.object(restore, "_scan_snapshot_directories", return_value=records),
                patch.object(restore, "list_source_snapshots", return_value=[]),
            ):
                snapshots, built = restore._timeshift_snapshots(config, FakeSource(False, []))

            self.assertIs(built, inventory)
            self.assertEqual(set(snapshots), {S1})
            self.assertEqual(set(snapshots[S1].subvolumes), {"@", "@home"})
            self.assertEqual(snapshots[S1].subvolumes["@"].uuid, f"original-{S1}-root")
    ...
    def test_ssh_target_inventory_keeps_snapshot_and_cache_on_same_remote_runner(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch.object(restore, "_scan_snapshot_directories", return_value={}),
                patch.object(restore, "list_source_snapshots", return_value=[]),
            ):
    ...
    def test_live_common_proof_requires_exact_readonly_cache_parent_chain(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            config = self.make_config(root, remote=False)
            source_meta = SubvolumeMeta("@", f"/snapshots/{S1}/@", uuid=f"original-{S1}-root")
            backup_meta = SubvolumeMeta(
                "@",
                f"{root}/snapshots/{S1}/@",
                uuid=f"backup-{S1}-root",
                received_uuid=f"send-{S1}-root",
                readonly=True,
            )
            cache_path = f"{config.source.cache_root}/{S1}/@"
            cache_index = BtrfsIndex(config.source.cache_root, "local")
            cache_index.add(
                SubvolumeMeta(
                    "@",
                    cache_path,
                    uuid=f"send-{S1}-root",
                    parent_uuid=f"original-{S1}-root",
                    readonly=True,
                )
            )
            inventory = SourceInventory(
            ...
            proven, reason = restore._live_payload_proof(
                config, source_meta, backup_meta, source_ops, inventory, S1, "@"
            )
            self.assertTrue(proven)
            self.assertIn("Parent UUID", reason)

            cache_index.by_path[cache_path].parent_uuid = None
            proven, reason = restore._live_payload_proof(
                config, source_meta, backup_meta, source_ops, inventory, S1, "@"
            )
            self.assertFalse(proven)
            self.assertIn("no Parent UUID", reason)

            cache_index.by_path[cache_path].parent_uuid = f"original-{S1}-root"
            cache_index.by_path[cache_path].readonly = False
            proven, reason = restore._live_payload_proof(
                config, source_meta, backup_meta, source_ops, inventory, S1, "@"
            )
            self.assertFalse(proven)
    ...
    def test_missing_clone_source_retries_only_exact_payload_once_as_full(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            config = self.make_config(root, remote=False)
            parent = self.backup(root, S1)
            current = self.backup(root, S2)
            parent_received_path = f"/restore-chain/{S1}/@home"
            received_path = f"/restore-chain/{S2}/@home"

            class FakeOps:
                def __init__(self):
                    self.endpoint = restore.CommandEndpoint.local("Timeshift target")
                    self.received_checks = 0
                    self.deleted = []

                def meta(self, path, *, name=None, required=True):
                    text_path = str(path)
                    if text_path == parent_received_path:
                        return SubvolumeMeta(
                            "@home",
                            text_path,
                            received_uuid=f"send-{S1}-home",
                            readonly=True,
                        )
                    if text_path == received_path:
                        self.received_checks += 1
                        if self.received_checks == 1:
                            return SubvolumeMeta("@home", text_path, uuid="partial", readonly=True)
                        if self.received_checks == 2:
                            return None
                        return SubvolumeMeta(
                            "@home",
                            text_path,
                            received_uuid=f"send-{S2}-home",
                            readonly=True,
                        )
                    raise AssertionError(f"unexpected meta path: {text_path}")
            ...
            failure = CommandError(
                ["btrfs", "receive"],
                1,
                stderr="ERROR: clone: cannot find source subvol e3990641-8f4f-a849-8527-9ebae7c3c377",
            )
            with patch.object(restore, "_run_restore_stream", side_effect=[failure, None]) as run_stream:
                restore._receive_restore_payload(
                    config,
                    SimpleNamespace(),
                    SimpleNamespace(environment=lambda: None),
                    ops,
                    backup=current,
                    backup_path=f"{current.path}/@home",
                    parent_backup=parent,
                    parent_path=f"{parent.path}/@home",
                    parent_received_path=parent_received_path,
                    receive_dir=f"/restore-chain/{S2}",
                    received_path=received_path,
                    subvolume_name="@home",
                    right_label="LOCAL TIMESHIFT RECEIVE",
                )

            self.assertEqual(run_stream.call_count, 2)
            self.assertEqual(run_stream.call_args_list[0].kwargs["parent_path"], f"{parent.path}/@home")
            self.assertIsNone(run_stream.call_args_list[1].kwargs["parent_path"])
            self.assertEqual(ops.deleted, [received_path])

    def test_unrelated_incremental_receive_failure_is_not_retried_as_full(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "target"
            config = self.make_config(root, remote=False)
            parent = self.backup(root, S1)
            current = self.backup(root, S2)
            parent_received_path = f"/restore-chain/{S1}/@"

            class FakeOps:
                endpoint = restore.CommandEndpoint.local("Timeshift target")

                def meta(self, path, *, name=None, required=True):
                    if str(path) == parent_received_path:
                        return SubvolumeMeta(
                            "@",
                            str(path),
                            received_uuid=f"send-{S1}-root",
                    ...
                    raise AssertionError("a non-parent failure must not inspect or delete a partial child")

                def delete(self, path):
                    raise AssertionError("ENOSPC must not trigger automatic deletion")

            failure = CommandError(["btrfs", "receive"], 1, stderr="ERROR: No space left on device")
            with patch.object(restore, "_run_restore_stream", side_effect=failure) as run_stream:
                with self.assertRaises(CommandError):
                    restore._receive_restore_payload(
                        ...
                        FakeOps(),
                        backup=current,
                        backup_path=f"{current.path}/@",
                        parent_backup=parent,
                        parent_path=f"{parent.path}/@",
                        parent_received_path=parent_received_path,
                        receive_dir=f"/restore-chain/{S2}",
                        received_path=f"/restore-chain/{S2}/@",
                        subvolume_name="@",
                        right_label="LOCAL TIMESHIFT RECEIVE",
                    )

            self.assertEqual(run_stream.call_count, 1)
