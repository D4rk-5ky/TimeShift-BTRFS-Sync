...
class CurrentStatePathTests(unittest.TestCase):
    ...
    def test_received_send_source_records_received_uuid_not_local_uuid(self):
        snapshot_root = "/source/snapshots"
        cache_root = "/source/cache"
        target_root = Path("/backup")
        name = "2026-07-15_05-00-02"
        ...
        mark_subvolume_synced(
            ...
            received_meta=SubvolumeMeta(
                "@",
                str(target_root / "snapshots" / name / "@"),
                uuid="new-destination-local-uuid",
            ...
            send_meta=SubvolumeMeta(
                "@",
                f"{cache_root}/{name}/@",
                uuid="received-copy-local-uuid",
                received_uuid="original-stream-uuid",
                parent_uuid=source_meta.uuid,
                readonly=True,
            ),
        )

        stored = state["snapshots"][name]["subvolumes"]["@"]
        self.assertEqual(stored["send_source_uuid"], "original-stream-uuid")
        self.assertNotEqual(stored["send_source_uuid"], "received-copy-local-uuid")
